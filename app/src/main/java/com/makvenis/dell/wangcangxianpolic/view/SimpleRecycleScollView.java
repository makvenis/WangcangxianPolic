...
import com.makvenis.dell.wangcangxianpolic.company.CompanyAdapter;

public class SimpleRecycleScollView extends RecyclerView {

    View view;
    private CompanyAdapter.MyViewHolder viewHolder;
    private LinearLayout itemLayout;
    private TextView deleteTextView, addTextView;
    private int lastX;
    private int lastY;
    ...
    public SimpleRecycleScollView(Context context) {
        super(context);
        scroller = new Scroller(context, new LinearInterpolator());
    }

    public SimpleRecycleScollView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        scroller = new Scroller(context, new LinearInterpolator());
    }

    public SimpleRecycleScollView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (state == 0) {//state=0表示菜单没被打开，state=1表示菜单被打开
                    ...
                    viewHolder = (CompanyAdapter.MyViewHolder) getChildViewHolder(view);//获得每一个Item的ViewHolder
                    itemLayout = viewHolder.layout;//获得ViewHolder相应的布局
                    deleteTextView = viewHolder.deleteTextView;//得到菜单栏里的控件，这里我们只有一个textview
                    addTextView = viewHolder.addTextView;
                    deleteWidth = deleteTextView.getWidth() + addTextView.getWidth();//获得侧滑菜单的宽度
                } else if (state == 1) {
                    View view1 = findChildViewUnder(x, y);
                    CompanyAdapter.MyViewHolder viewHolder1 = (CompanyAdapter.MyViewHolder) getChildViewHolder(view1);
